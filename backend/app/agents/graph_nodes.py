...
def _is_no_hotel_accommodation(accommodation: Any) -> bool:
    return any(keyword in str(accommodation or "") for keyword in NO_HOTEL_HINTS)
...
def _default_sop_required(request: Any) -> dict[str, bool]:
    ...
    hotels_required = bool(accommodation) and not _is_no_hotel_accommodation(accommodation)
...
def _append_tool_history(
    ...
    *,
    ...
    record = {
        ...
        "success": success,
        "reason": str(reason if reason is not None else agent_output.get("reasoning_summary") or ""),
        "summary": summary,
...
def _normalize_tool_result(
    *,
    ...
    """Normalize any capability tool response into one graph-level ToolResult."""
    raw_result = raw_result or {}
    items = [item for item in list(raw_result.get("items") or []) if isinstance(item, dict)]
    merged_warning = warning or raw_result.get("warning")
    text = str(raw_result.get("text") or failure_text)
    return {
        "tool_name": tool_name,
        "tool_input": tool_input,
        "success": bool(items),
        ...
        "meta": dict(raw_result.get("meta") or {}),
    }
...
def _execute_capability_tool(
    ...
    *,
    ...
    """Call one registered tool and return a normalized ToolResult."""
    tool_input = _tool_input_for_step(tool_name, state)
    try:
        tool = _get_capability_tool(tool_name)
...
def _run_capability_tool_node(
    ...
    *,
    ...
    """Executor-node adapter: call one registered tool and return only ToolResult."""
    tool_result = _execute_capability_tool(
        state,
        tool_name=tool_name,
        failure_text=failure_text,
        reason=str((state.get("agent_output") or {}).get("reasoning_summary") or ""),
    )
    return {
...
def _append_tool_history_from_result(
    state: TripPlannerState,
    tool_result: dict[str, Any],
) -> list[dict[str, Any]]:
    return _append_tool_history(
        state,
        tool_name=str(tool_result.get("tool_name") or ""),
        tool_input=dict(tool_result.get("tool_input") or {}),
        success=bool(tool_result.get("success")),
        summary=str(tool_result.get("text") or ""),
        result_count=len(tool_result.get("items") or []),
...
def _build_local_events_diagnostic(state: TripPlannerState) -> dict[str, Any]:
    context = state.get("gathered_context") or {}
    local_events = [item for item in (context.get("local_events") or []) if isinstance(item, dict)]
    was_called = any(
        isinstance(item, dict) and item.get("tool_name") == "search_local_events_tool"
        for item in state.get("tool_call_history") or []
    )
    interest_signal = _has_local_event_interest_signal(state)
    slow_pace_signal = _has_slow_pace_signal(state)
    candidate_gap = _attractions_insufficient(state)
    if was_called:
        status = "triggered"
        reason = _get_local_event_activation_reason(state)
    elif local_events:
        status = "triggered"
        reason = "existing_context"
    elif not all_required_sop_completed(state):
        status = "not_triggered"
        reason = "required_sop_first"
    elif not (interest_signal or slow_pace_signal or candidate_gap):
        status = "not_triggered"
        reason = "no_interest_or_pace_signal"
    else:
        status = "not_triggered"
        reason = "not_reached_before_planning"
    return {
        "status": status,
        "reason": reason,
        "interest_signal": interest_signal,
        "slow_pace_signal": slow_pace_signal,
        "candidate_gap": candidate_gap,
        "interest_keywords": _extract_local_event_interest_keywords(state) if (was_called or interest_signal) else [],
        "items_count": len(local_events),
    }


def build_agent_diagnostics(state: TripPlannerState) -> dict[str, Any]:
    """Build a public diagnostics snapshot from existing graph runtime state."""
    context = state.get("gathered_context") or {}
    transit_evidence = [
        item for item in (context.get("transit_evidence") or []) if isinstance(item, dict)
    ]
    filtered_by_transit = [
        {
        ...
        for item in transit_evidence
        if item.get("decision") == "drop_candidate"
    ]
    tool_history = [item for item in (state.get("tool_call_history") or []) if isinstance(item, dict)]
    return {
        "sop_required": dict(state.get("sop_required") or {}),
        "sop_completed": dict(state.get("sop_completed") or {}),
        "tool_calls": tool_history,
        "tool_failures": [
            item for item in tool_history if not item.get("success") or item.get("warning")
        ],
        ...
        "transit_filtered_candidates": filtered_by_transit,
        "local_events": _build_local_events_diagnostic(state),
...
def _failed_info_tool_names(state: TripPlannerState) -> set[str]:
    failed: set[str] = set()
    for record in state.get("tool_call_history") or []:
        if not isinstance(record, dict):
            continue
        tool_name = str(record.get("tool_name") or "")
        if tool_name in INFO_GATHERING_TOOL_NAMES and (record.get("success") is False or record.get("warning")):
...
def _rule_based_info_gathering_decision(state: TripPlannerState) -> Dict[str, Any]:
    ...
    sop_completed = state.get("sop_completed") or _default_sop_completed()
    failed_tools = _failed_info_tool_names(state)
    skipped_failed_tools: list[str] = []

    next_tool: str | None = None
    reason = "All mandatory information has been collected."

    required_steps = [
        ("attractions_required", "attractions_done", "search_attractions_tool", "Need baseline attractions before planning."),
        ("weather_required", "weather_done", "query_weather_tool", "Need weather to validate outdoor feasibility."),
        ("hotels_required", "hotels_done", "search_hotels_tool", "User needs hotel options before planning."),
        ("transit_required", "transit_done", "estimate_transit_time_tool", "Need transit evidence before submitting context."),
    ]
    for required_key, completed_key, tool_name, tool_reason in required_steps:
        if not sop_required.get(required_key) or sop_completed.get(completed_key):
            continue
        if tool_name in failed_tools:
            skipped_failed_tools.append(tool_name)
            continue
        next_tool = tool_name
        reason = tool_reason
        break

    if next_tool is None and not skipped_failed_tools and _should_search_local_events(state):
        if "search_local_events_tool" in failed_tools:
            skipped_failed_tools.append("search_local_events_tool")
        else:
            next_tool = "search_local_events_tool"
            reason = "Optional local events may improve the itinerary."

    if next_tool is not None and state.get("loop_count", 0) < state.get("max_loops", 10):
    ...
    if skipped_failed_tools and not next_tool:
        result = _build_info_gathering_node_result(
            state,
            action="continue",
            tool_name=None,
            reasoning_summary=f"Skipped repeated failed tools: {', '.join(skipped_failed_tools)}.",
            ready_for_planning=False,
        )
        result["force_exit_reason"] = (
            "信息搜集停止重复失败工具调用，未完成项："
            + "、".join(_missing_required_steps(state))
            + f"；已失败工具：{', '.join(skipped_failed_tools)}。"
        )
        return result
...
Base SOP tools (attractions, weather, and required hotels) are normally executed by SOP bootstrap before you are called. Only call them again when their required completion flag is still false and补查 is needed.
{
  "tools": [
    {
...
4. After bootstrap, prefer deciding optional local events, transit evidence, submit_context, or补查 missing required tools.
5. If unsure, choose continue.
6. Output JSON only, with keys: action, tool_name, reasoning_summary, ready_for_planning.
"""
    user_prompt = {
...
def _build_hard_restraints_block(state: TripPlannerState)->str:
    ...
    if request.accommodation:
        if _is_no_hotel_accommodation(request.accommodation):
            must_rules.append(f"住宿约束：本次行程为“{request.accommodation}”，必须不推荐酒店，且酒店预算必须为 0。")
        else:
...
def search_attractions_node(state: TripPlannerState) -> Dict[str, Any]:
    ...
    return _run_capability_tool_node(
        state,
        tool_name="search_attractions_tool",
        failure_text="Attraction lookup failed.",
        current_step="attractions_tool_executed",
    )
...
def query_weather_node(state: TripPlannerState) -> Dict[str, Any]:
    ...
    return _run_capability_tool_node(
        ...
        failure_text="Weather lookup failed.",
        current_step="weather_tool_executed",
    )
...
def search_hotels_node(state: TripPlannerState) -> Dict[str, Any]:
    ...
    return _run_capability_tool_node(
        ...
        failure_text="Hotel lookup failed.",
        current_step="hotels_tool_executed",
    )
...
def search_local_events_node(state: TripPlannerState) -> Dict[str, Any]:
    ...
    return _run_capability_tool_node(
        ...
        failure_text="Local events lookup failed.",
        current_step="local_events_tool_executed",
    )
...
def estimate_transit_time_node(state: TripPlannerState) -> Dict[str, Any]:
    ...
    return _run_capability_tool_node(
        ...
        failure_text="Transit estimation failed.",
        current_step="transit_tool_executed",
    )


def _merge_tool_result_into_state(
    state: TripPlannerState,
    tool_result: dict[str, Any],
    *,
    increment_loop: bool,
    ...
    tool_name = str(tool_result.get("tool_name") or "")
    items = [item for item in list(tool_result.get("items") or []) if isinstance(item, dict)]
    gathered_context = _copy_gathered_context(state)
    ...
    if tool_name == "search_attractions_tool":
        gathered_context["attractions"] = items
        sop_completed["attractions_done"] = bool(items)
        legacy_updates["attractions_data"] = json.dumps(items, ensure_ascii=False)
    elif tool_name == "query_weather_tool":
        gathered_context["weather"] = items
        sop_completed["weather_done"] = bool(items)
        legacy_updates["weather_data"] = json.dumps(items, ensure_ascii=False)
    elif tool_name == "search_hotels_tool":
        gathered_context["hotels"] = items
        if (state.get("sop_required") or {}).get("hotels_required"):
            sop_completed["hotels_done"] = bool(items)
        legacy_updates["hotel_data"] = json.dumps(items, ensure_ascii=False)
    elif tool_name == "search_local_events_tool":
        gathered_context["local_events"] = items
    elif tool_name == "estimate_transit_time_tool":
        gathered_context["transit_evidence"] = items
        sop_completed["transit_done"] = bool(items)
        for item in items:
            if item.get("decision") != "drop_candidate":
                continue
    ...
    return {
        **legacy_updates,
        ...
        "candidate_filter_notes": candidate_filter_notes,
        "tool_call_history": _append_tool_history_from_result(state, tool_result) if tool_result else list(state.get("tool_call_history") or []),
        "context_summary": next_state["context_summary"],
        "ready_for_planning": all_required_sop_completed(next_state),
        "loop_count": int(state.get("loop_count", 0)) + (1 if increment_loop and tool_result else 0),
...
def merge_tool_result_node(state: TripPlannerState) -> Dict[str, Any]:
    """Merge one unified ToolResult into graph state and history."""
    return _merge_tool_result_into_state(
        state,
...
def sop_bootstrap_node(state: TripPlannerState) -> Dict[str, Any]:
    ...
    tool_specs: list[tuple[str, str, str]] = [
        (
            "search_attractions_tool",
            "Attraction lookup failed.",
            "SOP bootstrap: 景点是基础必查项。",
        ),
        (
            "query_weather_tool",
            "Weather lookup failed.",
            "SOP bootstrap: 天气是基础必查项。",
        ),
    ]
    if required.get("hotels_required"):
        tool_specs.append(
            (
                "search_hotels_tool",
                "Hotel lookup failed.",
                "SOP bootstrap: 本次需要住宿，酒店是条件必查项。",
            )
        )
    ...
    return {
        "attractions_data": working_state.get("attractions_data"),
        "weather_data": working_state.get("weather_data"),
        "hotel_data": working_state.get("hotel_data"),
        "gathered_context": working_state.get("gathered_context"),
        "sop_completed": working_state.get("sop_completed"),
        "sop_required": working_state.get("sop_required"),
        "candidate_filter_notes": working_state.get("candidate_filter_notes"),
        "tool_call_history": working_state.get("tool_call_history"),
        "context_summary": working_state.get("context_summary"),
        "ready_for_planning": working_state.get("ready_for_planning"),
        "last_tool_result": None,
        "router_warning": None,
        "current_step": "sop_bootstrapped",
    }
...
def plan_trip_node(state: TripPlannerState) -> Dict[str, Any]:
    ...
    hotel_requirement_text = (
        "本次不住宿：不要推荐酒店，所有 day.hotel 必须为 null，budget.total_hotels 必须为 0。"
        if _is_no_hotel_accommodation(request.accommodation)
        else "本次需要住宿：每天推荐一个具体酒店，酒店类型必须匹配住宿偏好。"
    )
...
1. 每天安排2-3个景点
2. 每天必须包含早中晚三餐
3. 住宿规则必须服从用户住宿偏好；如果用户选择不住宿，则 day.hotel 必须为 null
4. 考虑景点之间的距离
5. 必须包含预算信息
...
def _build_fallback_day(request: Any, day_index: int) -> dict[str, Any]:
    ...
    return {
        ...
        "hotel": None if _is_no_hotel_accommodation(accommodation) else {
...
def _normalize_location(value: Any) -> dict[str, float]:
    if isinstance(value, dict):
        return {
            "longitude": _safe_float_or_none(value.get("longitude") or value.get("lng")) or 0.0,
            "latitude": _safe_float_or_none(value.get("latitude") or value.get("lat")) or 0.0,
        }
...
def _normalize_plan_days(plan: dict[str, Any], request: Any) -> None:
    ...
    for idx, day in enumerate(days):
        ...
        day.setdefault("description", f"第{idx + 1}天行程")
        day["transportation"] = str(day.get("transportation") or request.transportation)
        day["accommodation"] = str(day.get("accommodation") or request.accommodation)

        if _is_no_hotel_accommodation(request.accommodation):
            day["hotel"] = None
        elif isinstance(day.get("hotel"), dict):
            hotel = day["hotel"]
            hotel["name"] = str(hotel.get("name") or f"{request.city}{request.accommodation}")
            hotel["address"] = str(hotel.get("address") or request.city)
            hotel["location"] = _normalize_location(hotel.get("location")) if hotel.get("location") else None
            hotel["price_range"] = str(hotel.get("price_range") or "")
            hotel["rating"] = str(hotel.get("rating") or "")
            hotel["distance"] = str(hotel.get("distance") or "")
            hotel["type"] = str(hotel.get("type") or request.accommodation)
            hotel["estimated_cost"] = _safe_int_value(hotel.get("estimated_cost"), 0)

        normalized_attractions: list[dict[str, Any]] = []
        for attraction_index, attraction in enumerate(day.get("attractions") or []):
            if not isinstance(attraction, dict):
                continue
            name = str(attraction.get("name") or f"{request.city}景点{attraction_index + 1}")
            attraction["name"] = name
            attraction["address"] = str(attraction.get("address") or request.city)
            attraction["location"] = _normalize_location(attraction.get("location"))
            attraction["visit_duration"] = _safe_int_value(attraction.get("visit_duration"), 90)
            attraction["description"] = str(attraction.get("description") or name)
            attraction["category"] = str(attraction.get("category") or "景点")
            attraction["rating"] = _safe_float_or_none(attraction.get("rating"))
            attraction["photos"] = attraction.get("photos") if isinstance(attraction.get("photos"), list) else []
            attraction["poi_id"] = str(attraction.get("poi_id") or attraction.get("id") or "")
            attraction["ticket_price"] = _safe_int_value(attraction.get("ticket_price"), 0)
        ...
        for meal_index, meal in enumerate(day.get("meals") or []):
            if not isinstance(meal, dict):
                continue
            meal["type"] = str(meal.get("type") or ["breakfast", "lunch", "dinner"][min(meal_index, 2)])
            meal["name"] = str(meal.get("name") or meal["type"])
            meal["description"] = str(meal.get("description") or meal["name"])
            meal["estimated_cost"] = _safe_int_value(meal.get("estimated_cost"), 0)
            meal["location"] = _normalize_location(meal.get("location")) if meal.get("location") else None
            normalized_meals.append(meal)
        day["meals"] = normalized_meals

        if isinstance(day.get("timeline"), list):
            normalized_timeline: list[dict[str, Any]] = []
            for item in day["timeline"]:
                if not isinstance(item, dict):
                    continue
                item["start_time"] = str(item.get("start_time") or "09:00")
                item["end_time"] = str(item.get("end_time") or item["start_time"])
                item["activity_type"] = str(item.get("activity_type") or "activity")
                item["activity_name"] = str(item.get("activity_name") or item["activity_type"])
                item["duration"] = _safe_int_value(item.get("duration"), 0)
                item["location"] = _normalize_location(item.get("location")) if item.get("location") else None
                item["cost"] = _safe_int_value(item.get("cost"), 0)
...
def normalize_trip_plan_payload(plan: dict[str, Any], request: Any) -> dict[str, Any]:
    ...
    plan["city"] = str(plan.get("city") or request.city)
    ...
    if isinstance(plan.get("weather_info"), list):
        for idx, item in enumerate(plan["weather_info"]):
            ...
            weather_items.append(
                {
                    "date": str(item.get("date") or base_date.strftime("%Y-%m-%d")),
                    "day_weather": str(item.get("day_weather") or item.get("weather") or ""),
                    "night_weather": str(item.get("night_weather") or ""),
                    "day_temp": item.get("day_temp", item.get("temperature", 0)),
                    "night_temp": item.get("night_temp", 0),
                    "wind_direction": str(item.get("wind_direction") or ""),
                    "wind_power": str(item.get("wind_power") or ""),
                }
            )
    plan["weather_info"] = weather_items
    if not isinstance(plan.get("warnings"), list) and plan.get("warnings") is not None:
        plan["warnings"] = [str(plan["warnings"])]
    if not isinstance(plan.get("time_conflicts"), list):
        plan["time_conflicts"] = None
    else:
        plan["time_conflicts"] = [
            {
                "conflict_type": str(item.get("conflict_type") or "unknown"),
                "severity": str(item.get("severity") or "low"),
                "description": str(item.get("description") or "未提供冲突说明"),
                "affected_items": item.get("affected_items") if isinstance(item.get("affected_items"), list) else [],
                "day_index": item.get("day_index") if isinstance(item.get("day_index"), int) else None,
            }
            for item in plan["time_conflicts"]
            if isinstance(item, dict)
        ]
    if not isinstance(plan.get("budget_usage"), dict):
        plan["budget_usage"] = None
    _normalize_plan_days(plan, request)
    _rebuild_budget(plan)
    return plan
...
def info_gathering_router(state: TripPlannerState) -> str:
    if state.get("force_exit_reason"):
